...
import TP2.ASD.Statement.VariableFormAffectation.Array;
    ...
    public GenericRet toIR(SymbolTable symbolTable) throws TypeException {
        ...
        if (leftVar instanceof Array) {
            ...
            if (leftRet.getIr().isEmpty()) {
                ...
                result.getIr().appendCode(new LoadTab(resultIdent,
                                                      "%" + leftVar.getIdent(),
                                                      leftRet.getResult(),
                                                      leftSymbol.getIdent())); //TODO: change with index
            }
            leftVarIdent = resultIdent;

        } else {
            // maybe Symbol leftSymbol = symbolTable.lookup(leftVar.getIdent());
